...
#define SH_HEX_LENGTH			  3.0		// Length that hex lasts in seconds
#define SH_HEX_SPEED			  100.0		// Speed hexed player will move at
#define SH_HEALING_WAVE_RANGE	  750		// Range to heal teammates

#define SH_BIGBADVOODOO_DURATION  1
	...
	for ( new i = 0; i < numberofplayers; i++ )
	{
		...
		if ( parm[4] != get_user_team( targetid ) )
		{
			...
			if ( get_distance( origin, targetorigin ) < 85 )
			{
				...
				WC3_Damage( targetid, id, p_serpent_damage[iSkillLevel-1], CSW_SERPENTWARD, -1 );
