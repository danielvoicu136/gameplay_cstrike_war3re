...
WC3_Precache()
{
	...
	/*
		On some non-steam clients we will cause a crash error ... lets give some support for non-steam players , thx valve for understanding
	...
	*/
	...
	copy(g_szSounds[SOUND_ITEM_BUY], 63, "warcraft3/buy_items_pickup.wav");	   // Buy Item
	copy(g_szSounds[SOUND_ITEM_BUY_FAIL], 63, "warcraft3/buy_items_fail.wav"); // Buy Item Fail
	copy(g_szSounds[SOUND_MENU_SELECT], 63, "warcraft3/menu_press.wav");	   // Menu Select
...
WC3_CheckDev(id)
{
	...
	for (new i = 0; i < iTotalPlayers; i++)
	{
		...
		{
			client_print(0, print_chat, "%s The author Geesu is in this server", g_MODclient);
			bFound = true
		}
		...
		else if (equal(szAuthid, "STEAM_0:0:540768915"))
		{
			client_print(0, print_chat, "%s The developer xReforged is in this server", g_MODclient);
			bFound = true;
		}
...
WC3_ShowBar(id)
{
	...
	else if (g_MOD == GAME_DOD)
	{
		...
		else
		{
			...
			if (p_data[id][P_LEVEL] == 0)
			{
				pos += formatex(szRaceInfo[pos], 255, "%s^nExp: %d/%d^n", szRaceName, p_data[id][P_XP], XP_GetByLevel(p_data[id][P_LEVEL] + 1));
			}

			// User is under level 10
			else if (p_data[id][P_LEVEL] < MAX_LEVELS)
			{
				pos += formatex(szRaceInfo[pos], 255, "%s %L: %d^nExp: %d/%d^n", szShortRaceName, id, "WORD_LEVEL", p_data[id][P_LEVEL], p_data[id][P_XP], XP_GetByLevel(p_data[id][P_LEVEL] + 1));
			}

			// User is level 10
			else
			{
				pos += formatex(szRaceInfo[pos], 255, "%s %L: %d^nExp: %d^n", szShortRaceName, id, "WORD_LEVEL", p_data[id][P_LEVEL], p_data[id][P_XP]);
			}
		}
	}
...
WC3_ShowSpecInfo(id, iTargetID)
{
	...
	if (g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] > ITEM_NONE && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] != ITEM_MOLE)
	{
		...
		if (HasMoreThanOneRing && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] == ITEM_RING)
			iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^nEquip: %s x%d", szItemName, g_iTotalRings[iTargetID]);
		else
			iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^nEquip: %s", szItemName);
	...
	if (g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] > ITEM_NONE && g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] != ITEM_MOLE)
	{
		...
		else
		{
			// Then they have rings, lets print how many there are
			if (HasMoreThanOneRing && g_iShopMenuItems[iTargetID][ITEM_SLOT_TWO] == ITEM_RING)
				iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^nEquip: %s x%d", szItemName2, g_iTotalRings[iTargetID]);
			else
				iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^nEquip: %s", szItemName2);
		}
	}
	...
	if (g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO)
	{
		iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^n%L: %d   %L: %d", id, "CURRENT_HEALTH", get_user_health(iTargetID), id, "WORD_ARMOR", get_user_armor(iTargetID));
	}
	...
	iMsgPos += formatex(szMsg[iMsgPos], 512 - iMsgPos, "^n^n /shop   /race   /help");
...
public
WC3_Damage(iVictim, iAttacker, iDamage, iWeapon, iBodyPart)
{
...
public WC3_Death(iVictim, iKiller, iWeaponID, iHeadshot)
{
	...
	// Worgen Heal the Attacker 
	if (SM_GetSkillLevel(iKiller, SKILL_BLOODHUNT) > 0)
	{
		static iSkillLevel;
		iSkillLevel = SM_GetSkillLevel(iKiller, SKILL_BLOODHUNT);
		
		if(is_user_alive(iKiller)) { 
			
			new iHealthBonus = p_bloodhunt[iSkillLevel-1]; 
			new iMaxHealth = get_user_maxhealth( iKiller );
			if(get_user_health(iKiller) < iMaxHealth) 
			{ 
				set_user_health( iKiller, clamp(get_user_health( iKiller ) + iHealthBonus, 0, iMaxHealth));
				Create_ScreenFade( iKiller, (1<<10), (1<<10), (1<<12), 255, 0, 0, iHealthBonus );
			} 
		} 
...
WC3_PostSpawn(id)
{
	...
	// Worgen Enemy Skin 
	if ( SM_GetSkillLevel( id, SKILL_SAVAGEREND ) > 0 ) {
	static iSkillLevel;
	iSkillLevel = SM_GetSkillLevel( id, SKILL_SAVAGEREND );
		if(is_user_alive(id) && random_float( 0.0, 1.0 ) <= p_skinchanger[iSkillLevel-1]) 
		{ 
			SHARED_ChangeSkin( id, SKIN_SWITCH );
			client_print( id, print_chat, "%s You have your enemy skin !", g_MODclient );
		}
	}
...
WC3_NewSession(id)
{
	...
	if (is_user_bot(id))
	{
		...
		if (!p_data[id][P_RACE])
		{
			p_data[id][P_RACE] = random_num(1, RACE_ACTIVE_BOTS);
	...
	// set auto race after x spawns to all players

	if (iUserSpawnNumber[id] <= AUTO_SET_RACE_SPAWN && bUserHasAutoRace[id] == false)
	{
		iUserSpawnNumber[id]++;
	}

	if (!p_data[id][P_RACE] && bUserHasAutoRace[id] == false && iUserSpawnNumber[id] > AUTO_SET_RACE_SPAWN)
	{
		// Lets just give a race

		WC3_SetRace(id, random_num(1, 4));

		bUserHasAutoRace[id] = true;

		client_print(id, print_chat, "%s We auto select the race in spawn %d. Type /changerace to select another.", g_MODclient, iUserSpawnNumber[id]);
	}
...
public PrintMessageInfo(id)
{
	...
	client_cmd(id, "echo ^" *= -= -= -= -= -= -= -= -= -= = -= -= -= -= -= -= -= -= -= = -= -= -= -= -= -= -= -= -= -= -= -= *^" ");
	client_cmd(id, "echo ^" *= -= ^" ");
	client_cmd(id, "echo ^" *= -= Warcraft 3 Frozen Throne 2024 ^" ");
	client_cmd(id, "echo ^" *= -= Support : danielvoicu136@gmail.com ^" ");
	client_cmd(id, "echo ^" *= -= Update : 27.07.2024   ^" ");
	client_cmd(id, "echo ^" *= -= Visite : https://daeva.ro/  ^" ");
	client_cmd(id, "echo ^" *= -= Discord : https://discord.com/invite/RFjgXMuTDf  ^" ");
	client_cmd(id, "echo ^" *= -= ^" ");
	client_cmd(id, "echo ^" *= -= -= -= -= -= -= -= -= -= = -= -= -= -= -= -= -= -= -= = -= -= -= -= -= -= -= -= -= -= -= -= *^" ");
	client_cmd(id, "echo ");
}