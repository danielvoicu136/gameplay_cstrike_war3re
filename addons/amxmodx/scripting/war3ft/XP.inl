...
public bomb_defusing( defuser )
{
	...
	if ( !bHasBegunPlantingOrDefusing[defuser] )
	{
		...
		if ( iBonusXP != 0 )
		{
			client_print( defuser, print_chat, "%s You earned %d XP for defuse", g_MODclient, iBonusXP );
		}
	}
...
public bomb_planting( planter )
{
	...
	if ( !bHasBegunPlantingOrDefusing[planter] )
	{
		...
		if ( iBonusXP != 0 )
		{
			client_print( planter, print_chat, "%s You earned %d XP for plant", g_MODclient, iBonusXP );
		}
	}
...
public on_PlayerAction()
{
	...
	if (equal(sAction,"Spawned_With_The_Bomb"))
	{
		...
		{
			client_print( id, print_chat, "%s You earned %d XP for bomb", g_MODclient, iBonusXP ); 
		}
	}
	// Dropped bomb 
	else if (equal(sAction,"Dropped_The_Bomb"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{
			client_print( id, print_chat, "%s You lost %d XP for bomb", g_MODclient, -1 * iBonusXP );
		}
	}
	// Picked up bomb 
	...
	else if (equal(sAction,"Touched_A_Hostage"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{
			client_print( id, print_chat, "%s You earned %d XP for escape", g_MODclient, iBonusXP );
		}
	}
	// Hostage rescued 
	...
	else if (equal(sAction,"Killed_A_Hostage"))
	{
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{
			client_print( id, print_chat, "%s You lost %d XP for elimination", g_MODclient, -1 * iBonusXP );
		}
	}
	// VIP spawn 
	...
	{ 
		...
		if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
		{
			client_print( id, print_chat, "%s You earned %d XP for VIP", g_MODclient, iBonusXP );
		}
	}
	// VIP escaped 
...
XP_onDeath( iVictim, iAttacker, iWeaponIndex, iHeadshot )
{
	...
	if ( get_user_team( iAttacker ) == get_user_team( iVictim ) )
	{
		...
		if ( iBonusXP != 0 )
		{
			client_print( iAttacker, print_chat, "%s You lost %d XP for elimination", g_MODclient, -1 * iBonusXP );
		}
	...
	else
	{
		...
		{
			client_print( iAttacker, print_chat, "%s You earned %d XP for elimination", g_MODclient, iBonusXP );
		}
		...
		if ( iHeadshot )
		{
			...
			if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
			{
				client_print( iAttacker, print_chat, "%s You earned %d XP for headshot", g_MODclient, iBonusXP );
			}
		}
		...
		for ( new i = 0; i < MAXPLAYERS; i++ )
		{
			...
			if ( g_iDamageDealt[i][iVictim] > 0 && iAttacker != i )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_kill_obj ) )
				{
					client_print( i, print_chat, "%s You earned %d XP for assistance", g_MODclient, iBonusXP );
				}
		...
		if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
		{
			...
			else if ( g_iPlayerRole[iVictim] == PLR_VIP )
			{
				...
				if ( iBonusXP != 0 && get_pcvar_num( CVAR_wc3_show_objectives ) )
				{
					client_print( iAttacker, print_chat, "%s You earned %d XP for VIP elimination", g_MODclient, iBonusXP );
				}
			}
		}
	...
	for ( i = 0; i < numberofplayers; i++ )
	{
		...
		if ( get_user_team( id ) == iWinner )
		{
			...
			{				
				...
				client_print( id, print_chat, "%s You earned %d XP for win", g_MODclient, iBonusXP );
			}
		}
	}
}
