...
public on_ShowStatus( id )
{
	...
	if ( !g_freezeTime && get_pcvar_num( CVAR_wc3_show_player ) )
	{
		...
		lang_GetRaceName( p_data[iTarget][P_RACE], iTarget, szShortRaceName, 31, true );
		...
		if ( iViewerTeam == iTargetTeam || bShowAsTeammate )
		{
			...
			show_hudmessage( id, "%s ^n%d HP   %d AP ^n%s ^nLevel %d", szTargetName, get_user_health( iTarget ), get_user_armor( iTarget ), szShortRaceName, p_data[iTarget][P_LEVEL] );
		} 
