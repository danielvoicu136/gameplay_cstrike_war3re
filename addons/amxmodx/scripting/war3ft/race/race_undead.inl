...
#define EXPLOSION_MAX_DAMAGE	70 
#define EXPLOSION_RANGE			350
#define EXPLOSION_BLAST_RADIUS	350
	...
	for ( i = 0; i < numberofplayers; i++ )
	{
		...
		if ( iDistance < EXPLOSION_RANGE && iTeam != get_user_team( iTargetID ) && !bIgnoreDmg[iTargetID] )
		{
			...
			else
			{
				...
				// Damage them but not on low hp maps 
				if(g_bGlovesDisabled) 
				{
					if(is_user_alive(iTargetID)) set_user_health(iTargetID, 1); 
